  ...
  static async createExportBan(exportBan) {
    ...
    try {
      ...
      Logger.verbose(
        'Core',
        1,
        `Failed to send Discord Webhook: ${exportBan.ExportBanList.discordWebhook} for List "${exportBan.ExportBanList.name}". See logs above to see which webhook to take action on.`,
        err
      );
    }
  }
